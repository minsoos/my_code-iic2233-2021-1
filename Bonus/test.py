...
from api import registro, descargar_documento, entregar_consulta, NOMBRE, USERNAME
...
if __name__ == "__main__":
    code = registro(NOMBRE, USERNAME)
    print(code)
    consultas = [(consulta_1, PATRON_1), (consulta_2, PATRON_2), (consulta_3, PATRON_3),
        (consulta_4, PATRON_4), (consulta_5, PATRON_5), (consulta_6, PATRON_6)]
    

    for consulta in range(6):
        for documento in range(20, 30):
            descargar_documento(documento, f'documentos/documento{documento}.md')
            with open(f'documentos/documento{documento}.md') as f:
                texto = f.read()
            consulta_elem = consultas[consulta][0]
            patron = consultas[consulta][1]
            respuesta = consulta_elem(texto, patron)
            print(f"consulta:{consulta+1}, documento:{documento}")
            entregar_consulta(consulta+1, documento, patron, respuesta)
